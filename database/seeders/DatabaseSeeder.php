...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        self::seedCatalog();
        self::seedUsers();
        $this->command->info('Tablas inicializada con datos!');
    }

    private function seedCatalog()
    {
        DB::table('movies')->delete(); // Borra todos los registros existentes en la tabla
    ...
    {
        // Borra todos los registros de la tabla users
        //DB::table('users')->delete();

        // Crea usuarios de prueba
        $user1 = new User();
        $user1->name = 'Raul';
        $user1->email = 'raul@example.com';
        $user1->password = bcrypt('raul');
        $user1->save();

        $user2 = new User();
        $user2->name = 'Leiva';
        $user2->email = 'leiva@example.com';
        $user2->password = bcrypt('leiva');
        $user2->save();
